...
This software is designed to help automate encryption of secret texts. It will encrypt secret text with AES symmetric cypher algorythm also known as Rijndael ([wiki](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard)) and generate QRCode with encrypted data and BASE64 representation of encrypted data ([wiki](https://en.wikipedia.org/wiki/Base64)).

QRCode and Base64 representation of encrypted data then can be printed, stored in electronic formats (PDF and images) in locations which are not totally secure (or where data can be copied by unintended persons). This will allow for storage (in many possibly insecure locations) of the secret information, which should ensure it's survival.

Decrypt of QR codes can be done with companion android application (see [Papermache-android](https://github.com/vaal12-go/papermache-android) sister project and also in the application itself.
...
To stop application press "Stop server" button at the top menu in browser application or in console window press Ctrl-C and close browser window. No information will be stored on your computer.

## Technology

Papermache uses simple local http server and locally connected webbrowser. No data is sent to external machines on network and papermache can perfectly work on computers/phones, which are not connected to the network.

AES is used for encryption.

### Supported Operating systems
Tested on Windows OS. 
Plans for testing on Linux desktop family of OSes.
...
QR code can be decoded/decrypted in the application (update of Sep 2024) ("Read QR" menu in top menu of the application).
...
- [Gozxing](https://github.com/makiuchi-d/gozxing/) reading and geneation of QR codes
